...
const targetBrands = ['悦美时刻', 'ALLSMILE', 'Girlcult', 'KATO', '娜斯', '媞妃特', '彭世', 'CECILE MAIA', '简初'];

console.log('9个品牌的原始单元格数据:');
for (let row = range.s.r + 1; row <= range.e.r; row++) {
    const br = sheet[XLSX.utils.encode_cell({ r: row, c: brandIdx })];
    const brand = br && br.v != null ? String(br.v) : '';
    if (targetBrands.some(b => brand.includes(b))) {
        ...
        console.log('  品牌: ' + JSON.stringify(br ? br.v : 'undefined'));
        console.log('  百度指数: t=' + typeof (bi ? bi.v : undefined) + ' v=' + JSON.stringify(bi ? bi.v : undefined));
        console.log('  awareness: t=' + typeof (ai ? ai.v : undefined) + ' v=' + JSON.stringify(ai ? ai.v : undefined));
    }
}
